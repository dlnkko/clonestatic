...
export function buildPricingInstructions(allowedPrice: string | null, pricingDetail?: string | null): string {
  const detail = pricingDetail?.trim() || allowedPrice?.trim();
  if (detail) {
    if (pricingDetail && pricingDetail !== allowedPrice) {
      return `**PRICING (STRICT):** ${pricingDetail.trim()} Do NOT use any other dollar amount. Do NOT copy prices from the reference ad.`;
    }
...
export function synthesisPricingBlock(allowedPrice: string | null, pricingDetail?: string | null): string {
  return buildPricingInstructions(allowedPrice, pricingDetail);
}
