...
function replaceAdaptProductBlock(ctx: AdaptationContext): string {
  ...
  return `${header}
   ...
   - Replace product descriptions with **the same exact product as seen in the attached image(s)**
   - **Product presentation (CRITICAL — attached images are source of truth):** The hero must be **the same exact product as seen in the attached image(s)** — same container type (pouch vs bottle vs gummies), labels, logo, colors, shape, reproduced 1:1. Replicate reference **layout zones and pose** only — NEVER redesign the product or copy the reference competitor product form (e.g. do NOT draw a pill bottle when the attached images show a gummy pouch). (1) **Pose:** same orientation as reference for the user's actual product form; (2) **Position:** same inclination/tilt and direction; (3) **Placement:** same submerged/nestled relationship with background elements; (4) shadows, lighting, reflections as reference.
   ${peopleBlock}
...
- **Trust badge:** ${ctx.referenceTrustBadge.present ? 'Keep the seal layout slot only if user has a trust_badge image and/or scraped awards; never invent award name/year. If neither exists, omit seal or swap to other scraped proof.' : 'Only if reference had one.'}
...
- **STRICT DATA:** Product facts from scrape/approved copy only. Never invent awards, credentials, or claims; never copy competitor numbers/award titles from reference.
...
- **Awards / seals / "of the Year" / press badges:** Layout may mirror the reference seal slot, but lettering and claims MUST be real awards from scraped data or a provided trust_badge image. Never invent a parallel award for the user brand. If scrape has no awards and no badge image — omit or replace with other scraped proof.
