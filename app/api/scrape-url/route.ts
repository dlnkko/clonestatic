  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          details: 'Copy .env.example to .env.local, set FIRECRAWL_API_KEY, then restart the dev server (npm run dev).',
        },
        { status: 503 }
      );
    }
    ...
    try {
      ...
      try {
        ...
        const status = scrapeError.status ?? scrapeUrlError.status ?? scrapeUrlError.response?.status;
        const msg = scrapeUrlError.message || scrapeError.message;
        if (status === 401 || msg?.toLowerCase().includes('unauthorized') || (msg?.toLowerCase().includes('invalid') && (msg?.toLowerCase().includes('key') || msg?.toLowerCase().includes('token')))) {
          throw new Error('Invalid Firecrawl API key. In .env.local use a single line: FIRECRAWL_API_KEY=fc-your-key (no quotes). Get a valid key from https://firecrawl.dev dashboard and restart the dev server.');
        }
        throw new Error(msg || 'Firecrawl request failed.');
      }
    }
